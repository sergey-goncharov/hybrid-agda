...
Ã-Elim : ∀ {P : Ã → Set ℓ-P}
           ...
           (P-Set : ∀ x → IsSet (P x)) → (∀ x → P x)
         
Ã-Elim {ℓ-P} {P} P-[]~ P-eq P-Set [ s ]~ = P-[]~ s
Ã-Elim {ℓ-P} {P} P-[]~ P-eq P-Set (Ã-eq s≲t t≲s i) = subst≡→[]≡ {P = P} (P-eq s≲t t≲s) i
Ã-Elim {ℓ-P} {P} P-[]~ P-eq P-Set (Ã-Set {x} {y} p q i j) =
    Set→[]≡ Ã-Set P-Set (congP (Ã-Elim P-[]~ P-eq P-Set) p) (congP (Ã-Elim P-[]~ P-eq P-Set) q) i j

Ã-Elim-Prop : ∀ {P : Ã → Set ℓ-P} (P-[]~ : ∀ s → P [ s ]~) (P-Prop : ∀ x → IsProp (P x)) → (∀ x → P x)
Ã-Elim-Prop P-[]~ P-Prop = Ã-Elim P-[]~ (λ _ _ → P-Prop _ _ _) (λ x → prop-is-set (P-Prop x))

Ã-Rec : ∀ (f : ‖DirSeq‖ PO → B) (f-≲ : ∀ {s t : ‖DirSeq‖ PO} → s ≲ t → t ≲ s → f s ≡ f t) (B-Set : IsSet B) → (Ã → B)
Ã-Rec f f-≲ B-Set = Ã-Elim f (λ {s} {t} s≲t t≲s → trans (subst-const (Ã-eq {s} {t} s≲t t≲s)) (f-≲ s≲t t≲s)) (λ _ → B-Set)

Ã-Rec-Prop : ∀ (f : ‖DirSeq‖ PO → B) (B-Set : IsProp B) → (Ã → B)
Ã-Rec-Prop f B-Prop = Ã-Rec f (λ _ _ → B-Prop _ _) (prop-is-set B-Prop)

Ã-Rec2 : ∀ (f : ‖DirSeq‖ PO → ‖DirSeq‖ PO → B)
            (f-≲ : ∀ {s s′ t t′ : ‖DirSeq‖ PO} → s ≲ s′ → s′ ≲ s → t ≲ t′ → t′ ≲ t → f s t ≡ f s′ t′)
            (B-Set : IsSet B) → (Ã → Ã → B)
Ã-Rec2 f f-≲ B-Set =
   Ã-Rec (λ s → Ã-Rec (f s) (λ {t} {t′} t≲t′ t′≲t → f-≲ (≲-refl {s}) (≲-refl {s}) t≲t′ t′≲t) B-Set)
         (λ {s} {s′} s≲s′ s′≲s → →-≡ (Ã-Elim-Prop (λ t → f-≲ s≲s′ s′≲s (≲-refl {t}) (≲-refl {t})) (λ _ → B-Set)))
         (IsSet-∀ (λ _ → B-Set))
...
{-
‖DCPO‖-≾ : (A-⊥ : A) → Is-⊥ A-⊥ → ‖D-CompletePartialOrder‖ PO-≾
‖DCPO‖-≾ A-⊥ A-⊥-is-⊥ = record
  {
    ⊥ = [ (‖DirSeq‖-const A-⊥) ]~
  ; ⊥⊑x = λ {x} →
        Ã-Elim-Prop
           {P = λ x′ → proj₁ (LE [ (‖DirSeq‖-const A-⊥) ]~ x′)}
           (λ s n → ∣ n , (A-⊥-is-⊥ ( ‖DirSeq‖.seq s n)) ∣)
           (λ x′ → proj₂ (LE [ (‖DirSeq‖-const A-⊥) ]~ x′))
           x
  ; ⨆ = λ { (seq ‖⇗‖ dir) → {!!}}
  ; ⨆-ub = {!!}
  ; ⨆-lub = {!!}
...
ℕ→[]~-surj : ACω → IsSurj ℕ→[]~
ℕ→[]~-surj acω f = ‖‖-Rec ‖‖-prop (λ { (g , p) → ∣ g , →-≡ p ∣}) (acω _ λ n → []~-surj (f n))

≲-DirSeq : Set ℓ
≲-DirSeq = Σ (ℕ → ‖DirSeq‖ PO) (λ seq → ∀ n m → ∃[ k ] (seq n ≲ seq k × seq m ≲ seq k))

≲-DirSeq-≾ : ≲-DirSeq → DirSeq PO-≾
≲-DirSeq-≾ (seq , dir) = (λ n → [ seq n ]~) ⇗ dir

≲-DirSeq-≾-surj : ACω → ∀ (s : DirSeq PO-≾) → ‖ ∃[ s′ ] (DirSeq.seq (≲-DirSeq-≾ s′) ≡ DirSeq.seq s) ‖ 
≲-DirSeq-≾-surj acω (seq ⇗ dir) =
  ‖‖-map (λ { (s , p) →
                (s , λ n m →
                   proj₁ (dir n m)
                 , subst2 _≾_ (sym $ cong-app p n) (sym $ cong-app p $ proj₁ $ dir n m) (proj₁ $ proj₂ $ dir n m)
                 , subst2 _≾_ (sym $ cong-app p m) (sym $ cong-app p $ proj₁ $ dir n m) (proj₂ $ proj₂ $ dir n m))
                , p })
         (ℕ→[]~-surj acω seq)

≲-DirSeq-mono : ≲-DirSeq → Σ (‖DirSeq‖ PO → ‖DirSeq‖ PO) (λ f → ∀ {x y : ‖DirSeq‖ PO} → x ≲ y → f x ≲ f y) → ≲-DirSeq
≲-DirSeq-mono (seq , dir) (fun , mono) =
    fun ∘ seq , λ n m → (proj₁ $ dir n m) , (mono $ proj₁ $ proj₂ $ dir n m) , (mono $ proj₂ $ proj₂ $ dir n m)
